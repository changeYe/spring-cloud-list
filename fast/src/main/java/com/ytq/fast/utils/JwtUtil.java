...
public class JwtUtil {
    ...
    public String generateToken(String userName){
        ...
        String sign = JWT.create()
                ...
                .withExpiresAt(new Date(System.currentTimeMillis() + TimeUnit.SECONDS.toMillis(jwtProperties.getExpiration())))
    ...
    public DecodedJWT validateToken(String token){
        return decodedJWTMap.computeIfPresent(token, (key, decodedJWT) -> {
            Algorithm algorithm = getAlgorithm(jwtProperties.getSecretKey());
            JWTVerifier build = JWT.require(algorithm).build();
            DecodedJWT verify = null;
            try {
                verify = build.verify(key);
            }catch (Exception e){
